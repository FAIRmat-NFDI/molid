...
def set_sources(sources: tuple[str, ...]) -> None:
    ...
    if invalid:
        raise click.UsageError(
            f"Unknown sources: {invalid}. Use only: master cache api"
        )
...
def db_update(
    ...
    if not path:
        raise click.UsageError(
            "No DB path set; use `molid config set-master` or `--db-file`."
        )
    update_database(
        database_file=path,
...
@db.command("use")
@click.option(
    "--db-file", "db_path", default=None, help="Path to existing DB", type=str
)
...
@click.option(
    "--from-cid", type=int, default=None, help="Start from this CID (inclusive)"
)
@click.option("--limit", type=int, default=None, help="Max number of CIDs to enrich")
@click.option(
    "--synonyms/--no-synonyms",
    default=False,
    help="Also mine CAS from synonyms (validated)",
)
@click.option(
    "--sleep", "sleep_s", type=float, default=0.2, help="Delay between API calls (s)"
)
@click.option(
    "--timeout", "timeout_s", type=float, default=30.0, help="HTTP timeout (s)"
)
@click.option(
    "--retries", type=int, default=3, help="HTTP retries for transient failures"
)
def db_enrich_cas(
    db_path: str | None,
    from_cid: int | None,
...
@click.option(
    "--id-type",
    type=click.Choice(["inchikey", "smiles", "cid", "name", "molecularformula", "cas"]),
    default="inchikey",
)
def do_search(identifier: str, id_type: str) -> None:
    ...
    sources = [
        s.strip().lower()
        for s in (
            str(cfg.sources).split(",") if isinstance(cfg.sources, str) else cfg.sources
        )
    ]
    if "master" in sources and not cfg.master_db:
        raise click.UsageError(
            "No default master DB; use `molid config set-master` first."
        )
    if "cache" in sources and not cfg.cache_db:
        raise click.UsageError(
            "No default cache DB; use `molid config set-cache` first."
        )
